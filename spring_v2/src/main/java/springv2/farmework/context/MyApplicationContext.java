 ...
 * @author ccy
 */
public class MyApplicationContext extends DefaultListableBeanFactory implements BeanFactory {
...
//                System.out.println(obj);
        ...
        //todo 使用比较坑的两次循环 因为没有解决进行依赖注入的时候依赖没有初始化的情况  所以单独循环调用依赖注入方法populateBean 而不是放在getBean中
...
//                beanName = lowerFirstCase(field.getType().getSimpleName());
    ...
    public Object getBean(String beanName) {
        ...
        try{
            ...
            if(AopProxyUtils.isAopProxy(beanWrapper.getWrapperInstance())){
...
//        populateBean(beanName,instance);
    ...
    private AopConfig instantiationAopConfig(BeanDefinition beanDefinition) throws Exception {
        ...
        String pointCut = this.getConfig().getProperty("pointCut");
        //空格将类名和方法名分割开
        String[] before = this.getConfig().getProperty("aspectBefore").split("\\s");
        String[] after = this.getConfig().getProperty("aspectAfter").split("\\s");
        ...
        for(Method m : clazz.getMethods()){
            //public .* springv2\.demo\.service\..*Service\..*\(.*\)
            //public java.lang.string springv2.demo.service.impl.DemoService.get(java.lang.String);
            Matcher matcher = pattern.matcher(m.toString());
            //当前的类 满足切点表达式 添加到aop配置类中去
            if(matcher.matches()){
                //这里作为key的是原始对象的method
